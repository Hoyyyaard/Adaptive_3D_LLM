...
class VoteQuery(nn.Module):
    ...
    def forward(self, encode_xyz, encode_features):
        ...
        out = self.FFN_vote(encode_features) ## [bs,256,1024]
        vote_xyz = encode_xyz + out[:, :3, :].permute(0, 2, 1) ## [bs,1024,3]  Pseed+ΔPvote
        encode_features = encode_features + out[:, 3:, :] ## [bs,256,1024]
        ...
        query_xyz, query_features, _ = self.set_abstraction(         ## [bs,256,3] [bs,256,256]
            vote_xyz, 
            encode_features, 
            sample_inds
        )
