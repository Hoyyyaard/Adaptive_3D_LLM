...
class PointnetSAModuleVotes(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        # vis_pcd.points = o3d.utility.Vector3dVector(xyz.squeeze(0).cpu().numpy())
        # colors = np.ones_like(vis_pcd.points) * 0.5
        # colors[inds.squeeze(0).cpu().numpy()] = [1, 0, 0]  
...
class PointnetSAModuleVotesFP16(nn.Module):
    ''' Modified based on _PointnetSAModuleBase and PointnetSAModuleMSG
    with extra support for returning point indices for getting their GT votes '''
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        if not self.ret_unique_cnt:
            return new_xyz.half(), new_features.half(), inds
        else:
            return new_xyz.half(), new_features.half(), inds, unique_cnt
