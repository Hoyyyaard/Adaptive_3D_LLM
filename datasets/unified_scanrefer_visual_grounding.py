import sys
sys.path.append('/mnt/home/xiangtianhang/PG-LLM/third_party/LL3DA')
import re
...
class Dataset(ScanNetBaseDataset):
    ...
    def __init__(
        ...
        self.args = args
        self.task_name = 'scanrefer_viusal_grounding'
        ...
        self._tag_dataset(self.annotations, 'vg')
    ...
    def __getitem__(self, idx):
        ...
        # # FIXME: hard code for debug
        # # prompt['instruction'] = prompt['instruction'].format(caption=caption)
        # prompt = dict(
        ...
        if self.split == 'train':
            prompt = dict(
            instruction='### human: describe then localize the object from the 3D scene. ### assistant:',
            answer='the {category} is localized at {locations}',
        )
        else:
            description = self.annotations[idx]['description']
            prompt = dict(
            instruction=f'### human: localize the object from the 3D scene according to the description. Description:{description} ### assistant:',
            answer='the {category} is localized at {locations}',
        )

        ## USer: below is used for only input answer related pcds to model
        ...
        vg_gt_box_index = np.where(match_mask==1)[0].item()
        vg_box_angles = ret_dict["gt_box_angles"][match_mask==1]

        # for the reference object location 
        ...
        if self.split == 'train':
            ...
            locations = vg_gt_box_discretized
            response = prompt['answer'].format(category=category, locations=locations)
        else: 
            category = category_name
            locations = vg_gt_box_discretized
            response = prompt['answer'].format(category=category, locations=locations)
        ...
        #     if vg_gt_box_index not in indices:
        #         idx = np.random.randint(0, len(indices))
        #         indices[idx] = vg_gt_box_index
...
if __name__ == "__main__":
    ...
    def vis_box_and_vg_box():
        ...
        save_dir_base = "/mnt/home/xiangtianhang/PG-LLM/third_party/LL3DA/temp/visualization/vg"
    ...
    # vis_box_and_vg_box()
    count_max_object_number_in_a_scene()
