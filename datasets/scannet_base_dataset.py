...
class ScanNetBaseDataset(Dataset):
    ...
    def _get_scan_data(self, scan_name,):
        ...
        # self.visualization_tgt_obj_bboxs(box_corners.astype(np.float32), point_cloud)
    ...
    def _get_scan_data_adaptive(self, scan_name):
        ...
        semantic_labels = np.load(
            os.path.join(f'{self.data_path}_dense', scan_name) + "_sem_label.npy"
        )
        ## bbox = np.array([(xmin+xmax)/2, (ymin+ymax)/2, (zmin+zmax)/2, xmax-xmin, ymax-ymin, zmax-zmin, label_id, obj_id-1]
        instance_bboxes = np.load(os.path.join(f'{self.data_path}_dense', scan_name) + "_aligned_bbox.npy")
